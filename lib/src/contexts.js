...
/** @template TString1 @template TString2 @typedef {import('./types.js').AugmentString<TString1, TString2>} AugmentString */
/** @template TModel @typedef {import('./types.js').TableJoinMetadata<TModel>} TableJoinMetadata */
/** @template TModel @typedef {import('./types.js').WhereBuilderFunction<TModel>} WhereBuilderFunction */
/** @template TModel @typedef {import('./types.js').OrderByBuilderFunction<TModel>} OrderByBuilderFunction */
/** @template TModel @typedef {import('./types.js').GroupByBuilderFunction<TModel>} GroupByBuilderFunction */
/** @template TModel @typedef {import('./types.js').ExtractModel<TModel>} ExtractModel */
/** @typedef {import('./types.js').TableContextOptions} TableContextOptions */
/** @typedef {import('./types.js').OnConnectHandler} OnConnectHandler */
/** @typedef {import('./types.js').SuccessHandler} SuccessHandler */
/** @typedef {import('./types.js').FailHandler} FailHandler */
/** @typedef {import('./types.js').GroupByAliases} GroupByAliases */
/** @typedef {import('./types.js').AbstractModel} AbstractModel */
...
export class MySqlTableContext {
    ...
    async _query(cmd, args = undefined) {
        ...
        try {
            ...
            this._pool.emit(`${MySqlTableContext.EVENT_TABLE_CONTEXT_QUERY}-${this._table}`, {
                ...
                table: `[${this._cnn.config.database}].[dbo].[${this._table}]`,
                cmdRaw,
                cmd,
                args
            });
    ...
    */
     ...
     * @param {FailHandler=} fail Function that executes when a query command failed execution on this context.
     */
    onQuery(success, fail = undefined) {
        if (fail) {
            this._pool.addListener(`${MySqlTableContext.EVENT_TABLE_CONTEXT_QUERY_FAILED}-${this._table}`, fail);
        }
     ...
     * @param {FailHandler=} fail Function that executes when an insert command failed execution on this context.
     */
    onInsert(success, fail = undefined) {
        if (fail) {
            this._pool.addListener(`${MySqlTableContext.EVENT_TABLE_CONTEXT_INSERT_FAILED}-${this._table}`, fail);
        }
     ...
     * @param {FailHandler=} fail Function that executes when an update command failed execution on this context.
     */
    onUpdate(success, fail = undefined) {
        if (fail) {
            this._pool.addListener(`${MySqlTableContext.EVENT_TABLE_CONTEXT_UPDATE_FAILED}-${this._table}`, fail);
        }
     ...
     * @param {FailHandler=} fail Function that executes when a delete command failed execution on this context.
     */
    onDelete(success, fail = undefined) {
        if(fail) {
            this._pool.addListener(`${MySqlTableContext.EVENT_TABLE_CONTEXT_DELETE_FAILED}-${this._table}`, fail);
        }
